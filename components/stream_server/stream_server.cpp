...
void StreamServerComponent::setup() {
  ...
  // The make_unique() wrapper doesn't like arrays, so initialize the unique_ptr
  // directly.
  ...
  socklen_t bind_addrlen =
      socket::set_sockaddr_any(reinterpret_cast<struct sockaddr *>(&bind_addr),
                               sizeof(bind_addr), htons(this->port_));
#endif

  this->socket_ = socket::socket_ip(SOCK_STREAM, PF_INET);
  this->socket_->setblocking(false);
  this->socket_->bind(reinterpret_cast<struct sockaddr *>(&bind_addr),
                      bind_addrlen);
...
void StreamServerComponent::accept() {
  ...
  std::unique_ptr<socket::Socket> socket = this->socket_->accept(
...
void StreamServerComponent::cleanup() {
  ...
  auto last_client = std::partition(this->clients_.begin(),
                                    this->clients_.end(), discriminator);
...
void StreamServerComponent::read() {
  ...
  while ((available = this->stream_->available()) > 0) {
    ...
    if (free == 0) {
      // Only overwrite if nothing has been added yet, otherwise give flush() a
      // chance to empty the buffer first.
      if (len > 0)
        return;

      ESP_LOGE(TAG, "Incoming bytes available, but outgoing buffer is full: "
      ...
      for (Client &client : this->clients_) {
        if (client.position < this->buf_tail_) {
          ESP_LOGW(TAG, "Dropped %u pending bytes for client %s",
                   this->buf_tail_ - client.position,
    ...
    len = std::min<size_t>(
        available, std::min<size_t>(this->buf_ahead(this->buf_head_), free));
    this->stream_->read_array(&this->buf_[this->buf_index(this->buf_head_)],
                              len);
    this->buf_head_ += len;
  }
}
...
void StreamServerComponent::flush() {
  ...
  for (Client &client : this->clients_) {
    ...
    // Split the write into two parts: from the current position to the end of
    // the ring buffer, and from the start of the ring buffer until the head.
    // The second part might be zero if no wraparound is necessary.
    struct iovec iov[2];
    iov[0].iov_base = &this->buf_[this->buf_index(client.position)];
    iov[0].iov_len = std::min(this->buf_head_ - client.position,
                              this->buf_ahead(client.position));
    ...
    if ((written = client.socket->writev(iov, 2)) > 0) {
      ...
      ESP_LOGE(TAG, "Failed to write to client %s with error %d!",
               client.identifier.c_str(), errno);
    }
...
void StreamServerComponent::write() {
  ...
  for (Client &client : this->clients_) {
    ...
    if (read == 0 || errno == ECONNRESET) {
      ...
      ESP_LOGW(TAG, "Failed to read from client %s with error %d!",
               client.identifier.c_str(), errno);
    }
  }
}
